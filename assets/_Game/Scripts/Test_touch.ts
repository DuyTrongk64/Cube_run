...
export class Test_touch extends Component {
    @property(Node)
    public Camera_Base: Node;

    private touch_moved: boolean;
    ...
    onLoad () {
        ...
        this.node.on(Node.EventType.TOUCH_CANCEL, this.onTouchCancelled, this, true);
    ...
    Cancel_Inner_Touch(event){
        ...
        if (deltaMove.mag() > 7) {
            if (!this.touch_moved && event.target !== this.node) {
                ...
                var cancelEvent = event.EventTouch(event.getTouches(), event.bubbles);
